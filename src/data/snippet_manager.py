...
class SnippetManager(QObject):
    ...
    def perform_search(self, query: str) -> List:
        ...
        if not query:
            return [snippet for snippet in self.get_snippets()]
        if "*" in query:
    ...
    def get_snippet_types(self) -> List:
        ...
        """
        snippet_types: pl.DataFrame = DatabaseManager().read_database(
            table_name="snippets",
            columns="type",
            group="type",
        )
    ...
    def get_snippets(
        ...
        return DatabaseManager().read_database("snippets", f"{columns}").to_dicts()
    ...
    def save_snippet(self, new_snippet: dict) -> None:
        ...
        DatabaseManager().insert_data("snippets", columns, values)
